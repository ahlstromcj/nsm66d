...
be more to come. Some options might not work on Windows.
 ...
 --clean             Delete the usual derived files from the project. Also
                     do "git checkout doc/nsm66d-app-guide.pdf"
...
#
#  rm -f doc/latex/*.log
#  echo "Build products removed from the nsm66d/build directory."
#  git checkout doc/nsm66d-app-guide.pdf tests/data/fooout.rc
...
if test "$DOINSTALL" = "yes" ; then
   ...
   if test "$USERID" = 0 ; then
      cd build
      echo "Installing the nsm66d application..."
      meson install
      cd ..
   else
      echo "UID $USERID. We want you as root to install the nsm66d applications..."
   fi
...
if test "$DOUNINSTALL" = "yes" ; then
   ...
   USERID=$(id -u)
   if test "$USERID" = 0 ; then
      echo "Uninstalling the nsm66d application..."
...
#        rm -rf "$INSTALL_PREFIX/man/man1/$NSM66.1"
      ...
      echo "UID $USERID. We want you as root to uninstall the nsm66d applications..."
   fi
